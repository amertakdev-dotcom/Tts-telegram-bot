...
const { sendFormatted } = require("./markdown");
...
async function handlePhotoMessage(bot, msg) {
  ...
  try {
            ...
            content: [
              {
                type: "text",
                text:
                  "Extract ALL text visible in this image exactly as written, preserving line breaks. Keep Khmer, English, and numbers exactly as shown. Reply with ONLY the extracted text and nothing else — no explanation, no translation. Note: Only scan text from image 100% no describe, no tutorial, only send the text from image to user.",
              },
    ...
    if (!extracted) {
      ...
      await sendFormatted(bot, chatId, `📝 អត្ថបទដែលបានស្រង់ចេញ:\n\n${extracted}`);
    }
  } catch (err) {
    // Log the real Groq error body (invalid model, bad key, rate limit,
    // etc.) — this is the detail that was previously swallowed, which is
    // why OCR failures were silent and hard to diagnose.
    console.error("❌ OCR error:", err.message);
    if (err.response?.data) {
      console.error("❌ OCR error detail:", JSON.stringify(err.response.data));
    }

    let userMessage = "❌ បរាជ័យក្នុងការស្កេនរូបភាព។ សូមព្យាយាមម្តងទៀត។";
    const groqErrorCode = err.response?.data?.error?.code;
    if (groqErrorCode === "model_decommissioned" || err.response?.status === 404) {
      userMessage = "❌ Vision model មិនអាចប្រើបានទេ (ត្រូវបានផ្លាស់ប្តូរ)។ សូមទាក់ទងអ្នកអភិវឌ្ឍន៍។";
    } else if (err.response?.status === 401) {
      userMessage = "❌ Groq API key មិនត្រឹមត្រូវទេ។";
    } else if (err.response?.status === 429) {
      userMessage = "⏱ ប្រើប្រាស់ច្រើនពេក សូមរង់ចាំបន្តិច រួចព្យាយាមម្តងទៀត។";
    }

    await bot
      .editMessageText(userMessage, {
        chat_id: chatId,
